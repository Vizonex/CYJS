...
class Counter:
    ...
    def on_promise(self, ctx:Context, hook:PromiseHookType, promise:Promise, parent:object):
    ...
    def is_equal_to(self, init: int, before: int, after: int, resolve: int) -> bool:
        return self.init == init and self.before == before and self.after ==after and self.resolve == resolve
